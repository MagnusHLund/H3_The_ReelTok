...
namespace reeltok.api.users.Repositories
{
    public class SubscriptionsRepository : ISubscriptionsRepository
    {
        ...
        {
            return await _context.Subscriptions.Where(s => s.SubDetails.UserId == userId).Select(s => s.SubDetails.SubscribingToUserId)
        ...
        public async Task<List<Guid>> GetSubscriptionsByUserIdAsync(Guid userId)
        {
            return await _context.Subscriptions.Where(s => s.SubDetails.SubscribingToUserId == userId)
                .Select(s => s.SubDetails.UserId).ToListAsync().ConfigureAwait(false)
                ?? new List<Guid>();
        }
        ...
        public async Task<bool> RemoveUserFromSubscriptionAsync(Guid userId, Guid subscriptionUserId)
        {
            SubscriptionEntity subscription = await _context.Subscriptions
                .FirstOrDefaultAsync(s => s.SubDetails.UserId == userId && s.SubDetails.SubscribingToUserId == subscriptionUserId)
                .ConfigureAwait(false)
